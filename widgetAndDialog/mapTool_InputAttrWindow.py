...
class inputAttrWindowClass(QDialog,Ui_Dialog):
    ...
    def addFeature(self):
        ...
        # 在添加要素之前记录要素数量
        before_feature_count = self.mapTool.editLayer.featureCount()
        self.mapTool.editLayer.addFeature(self.feat)
        # 在添加要素之后记录要素数量
        after_feature_count = self.mapTool.editLayer.featureCount()
        print(f"要素添加前数量: {before_feature_count}")
        print(f"要素添加后数量: {after_feature_count}")
