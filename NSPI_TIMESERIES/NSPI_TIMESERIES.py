...
class CloudMethod:
    def __init__(self, L1_path, L2_path, L3_path, save_path):
        # ******************************************************************************************************
        ...
        temp_file = save_path  ###选择一个文件夹，返回文件夹路径。
        #temp_file = r"D:\code\test2\123"
        ...
        ind_patch1 = np.zeros((n_nl * n_ns, 4), dtype=int)  # np.int改为了int
        ...
        for i_ns in range(0, n_ns):
            for i_nl in range(0, n_nl):
                ...
                location[n_ns * i_nl + i_ns, 1] = ind_patch1[n_ns * i_nl + i_ns, 1] - ind_patch1[
                    n_ns * i_nl + i_ns, 0] + \
                                                  location[n_ns * i_nl + i_ns, 0]
                ...
                location[n_ns * i_nl + i_ns, 3] = ind_patch1[n_ns * i_nl + i_ns, 3] - ind_patch1[
                    n_ns * i_nl + i_ns, 2] + \
                                                  location[n_ns * i_nl + i_ns, 2]
        ...
        for isub in range(0, n_nl * n_ns):
            ...
            mark = np.zeros(
                (n_image, location[isub, 3] - location[isub, 2] + 1, location[isub, 1] - location[isub, 0] + 1)).astype(
                int)
            ...
            for i_img in range(0, n_image):
                ...
                clf = MiniBatchKMeans(n_clusters=NUM_CLASSES, random_state=0, max_iter=ITERATIONS,
                        ...
                        for j in range(location[isub, 2], location[isub, 3] + 1):
                            ...
                            if 0 < gap[j, i] < 10:
                                if mask2[j, i] == 0:
                                    ...
                                    classij = class2[j, i]
                                    extent = np.ceil(
                                        0.5 * (np.sqrt(min_similar) - 1))  # compute the minimum window size
                                    max_window = np.round(
                                        max([nl, ns]) * 0.25)  # maximum size of half seach window is 1/4 of image size
                                    ...
                                    if num_outcloud > 0:
                                        max_window_p = np.min(
                                            spatial_position[ind_outcloud])  # find the possibel maximum half window
                                    ...
                                    if max_window_p == max_window:
                                        ...
                                        ind_class = np.logical_and(np.logical_and(sub_gap == 0, sub_mask2 == 0),
                                                                   sub_class == classij)
                                    ...
                                    while ind_success == 0 and start_w <= end_w:
                                        ...
                                        sub_mask2 = mask2[b1:b2 + 1, a1:a2 + 1]
                                        sub_class = class2[b1:b2 + 1, a1:a2 + 1]
                                        ind_common = np.logical_and(np.logical_and(sub_gap == 0, sub_mask2 == 0),
                                                                    sub_class == classij)
                                    ...
                                    if c_common >= 2.0 * min_similar:  # if same-class pixels more than required sample size
                                        ...
                                        disi = np.sqrt(
                                            (it - col_wind[ind_common]) ** 2 + (jt - row_wind[ind_common]) ** 2)
                                        sub_on_common = np.zeros((num_band, c_common)).astype(float)
                                        sub_off_common = np.zeros((num_band, c_common)).astype(float)
                                        for iband in range(0, num_band):
                                            sub_on_common[iband, :] = (sub_on[iband, :, :])[ind_common] - fine2[
                                                iband, j, i]
                                            sub_off_common[iband, :] = (sub_on[iband, :, :])[ind_common] - \
                                                                       sub_off[iband, :, :][
                                                                           ind_common]
                                        ...
                                        for iband in range(0, num_band):
                                            ...
                                            predict_2 = fine2[iband, j, i] + np.sum(
                                                np.multiply((similar_off - similar_on), weight))
                                        ...
                                        if c_common >= 3 and extent >= max_window:
                                            ...
                                            if num_good_before > 0 and num_good_after > 0:
                                                ...
                                                for iband in range(0, num_band):
                                                    ...
                                                    fine0[i_img * num_band + iband, j - location[isub, 2], i - location[
                                                        isub, 0]] = (T2 * before_band + T1 * after_band) / (T2 + T1)
                                            else:
                                                fine0[i_img * num_band:((i_img + 1) * num_band), j - location[isub, 2],
                                                i - location[isub, 0]] = fine2[:, j, i]

                                            mark[i_img, j - location[isub, 2], i - location[isub, 0]] = 3 + 10 * i_input
                                            note = 1
                                            gap[j, i] = -2

                    ind_unfilled = np.where(
                        gap[location[isub, 2]:location[isub, 3] + 1, location[isub, 0]:location[isub, 1] + 1] > 0)
